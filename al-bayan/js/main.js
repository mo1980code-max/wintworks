/* ============================================================
   Al-Bayan — Main site JS (vanilla JS, no jQuery)
...
(function () {
  ...
  var DATA = window.getSiteData ? window.getSiteData() : window.ALBAYAN_DEFAULTS;
  var state = {
    cityId: (window.ALBAYAN_STORE ? window.ALBAYAN_STORE.get("albayan-city") : localStorage.getItem("albayan-city")) || DATA.prayerSettings.defaultCityId,
    method: (window.ALBAYAN_STORE ? window.ALBAYAN_STORE.get("albayan-method") : localStorage.getItem("albayan-method")) || DATA.prayerSettings.method,
    madhab: (window.ALBAYAN_STORE ? window.ALBAYAN_STORE.get("albayan-madhab") : localStorage.getItem("albayan-madhab")) || DATA.prayerSettings.asrMadhab,
    times: null
  };
  ...
  function initQuranPage() {
    ...
    function open(n) {
      ...
      if (!s) return;
      current = n;
      window.ALBAYAN_STORE.set("albayan-last-surah", String(n));
      textBox.innerHTML = "";
    ...
    var lastSaved = parseInt(window.ALBAYAN_STORE.get("albayan-last-surah") || "1", 10);
  ...
  function initQuranDarkMode() {
    ...
    apply(window.ALBAYAN_STORE.get("albayan-quran-dark") === "1");
    btn.addEventListener("click", function () {
      var on = !document.body.classList.contains("quran-dark");
      window.ALBAYAN_STORE.set("albayan-quran-dark", on ? "1" : "0");
      apply(on);
    });
  }
  ...
  function initDonateForm() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      var records = [];
      try { records = JSON.parse(window.ALBAYAN_STORE.get("albayan-donations") || "[]"); } catch (err) { records = []; }
      records.push({ id: Date.now(), name: donor, cause: cause ? cause.title : "General", amount: amount, date: new Date().toISOString() });
      window.ALBAYAN_STORE.set("albayan-donations", JSON.stringify(records));
