/* ============================================================
   Al-Bayan — Admin panel (demo, English) — vanilla JS
...
(function () {
  ...
  $("#loginForm").addEventListener("submit", function (e) {
    ...
    if (pin === "albayan01") {
      window.ALBAYAN_STORE.set("albayan-admin-ok", "1");
      showPanel();
      loadAll();
      toast("Welcome back to the admin panel");
    } else {
      toast("Incorrect access code (demo: albayan01)", "error");
    }
  });

  $("#logoutBtn").addEventListener("click", function () {
    window.ALBAYAN_STORE.del("albayan-admin-ok");
  ...
  if (window.ALBAYAN_STORE.get("albayan-admin-ok") === "1") { showPanel(); loadAll(); }
  ...
  function renderDonations() {
    ...
    tbody.innerHTML = "";
    var records = [];
    try { records = JSON.parse(window.ALBAYAN_STORE.get("albayan-donations") || "[]"); } catch (e) { records = []; }
    if (!records.length) {
  ...
  function save() {
    ...
    if (window.ALBAYAN_UI && window.ALBAYAN_UI.refresh) window.ALBAYAN_UI.refresh();
  }
  ...
  $("#e-addBtn").addEventListener("click", function () {
    ...
    data.events.push({
      ...
      guestBio: "Speaker at Al-Bayan Islamic Center.",
