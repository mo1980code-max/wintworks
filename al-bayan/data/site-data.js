/* ============================================================
   Al-Bayan — Central site data (defaults, English version)
...
window.ALBAYAN_DEFAULTS = {
  general: {
    siteName: "Al-Bayan Islamic Center",
    siteNameEn: "AL-BAYAN ISLAMIC CENTER",
    slogan: "A home for every Muslim",
    phone: "+20 100 000 0000",
    email: "info@albayan-center.org",
...
window.getSiteData = function () {
  var raw = window.ALBAYAN_STORE.get("albayan-site-data");
  if (!raw) return JSON.parse(JSON.stringify(ALBAYAN_DEFAULTS));
  try {
    var saved = JSON.parse(raw);
    var merged = JSON.parse(JSON.stringify(ALBAYAN_DEFAULTS));
    ...
    return JSON.parse(JSON.stringify(ALBAYAN_DEFAULTS));
  }
};

window.saveSiteData = function (data) {
  window.ALBAYAN_STORE.set("albayan-site-data", JSON.stringify(data));
};

window.resetSiteData = function () {
  window.ALBAYAN_STORE.del("albayan-site-data");
};
